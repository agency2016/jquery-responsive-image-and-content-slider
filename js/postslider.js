...
$.fn.slider = function(config){
    ...
    $(this).each(function(){
        ...
        var small_traverse_btn = '<ul class="sliderlist" style="z-index:8888;position: absolute;left: 0;top:'+parseInt(($(this).height()-20))+'px;">';
        for(loop = 0;loop <pages_list.length;loop++){
            ...
            if(loop != 0){
                ...
                small_traverse_btn += '<li style="padding:2px;display: inline;list-style-type: none;"><a href="#" style="decoration: none" class="sliderlistlia" data-item ="'+loop+'">'+'<img src="icons/off.png" style="width: 12px;height: 12px;">'+'</a></li>';
            }
        }
        small_traverse_btn += '</ul>';

        $(this).append(small_traverse_btn);
        var margin_ = parseInt((postsliderwidth*options.per_page)/2-(14*(pages_list.length)));
    ...
    $('.sliderlistlia').click(function(event){
        ...
        if(current != id){
            ...
            var images       =  $(this).parents('.sliderlist').siblings('.postslider');
            $(images).removeClass('active').slideUp(100);
            var length_      =  images.length;
            var page_number  = Math.ceil(length_/options.per_page);
            var current_page = 0;
            ...
            $(this).html('<img src="icons/off.png" width="12px" height="12px"/> ');
            $(this).parents('.sliderlist').find('.sliderlistlia[data-item !='+next+']').html('<img src="icons/on.png" width="12px" height="12px"/> ');
        }
