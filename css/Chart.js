            ...
            if (animationDecimal >= 1) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (!(typeof(data[i].value)=='undefined')){
                        ...
                        jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["ARC", midPosX, midPosY, 0, calculateOffset(config, 1*data[i].value, calculatedScale, scaleHop), startAngle - angleStep, startAngle, data[i].answers, 1*data[i].value, cumvalue, totvalue, angleStep, i];
...
//We accept multiple data sets for radar charts, so show loop through each set
        ...
        function drawScale() {
            ...
            for (var k = 0; k < data.labels.length; k++) {
                ...
                ctx.fillStyle = data.labelsfillColor[k];
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (!(typeof(data[i].value)=='undefined')){
                    ...
                    if (animationDecimal >= 1) {
                        ...
                        jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["ARC", midPieX, midPieY, 0, pieRadius, cumulativeAngle - segmentAngle, cumulativeAngle, data[i].answers, 1*data[i].value, cumvalue, totvalue, segmentAngle, i];
                        if (config.inGraphDataShow) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (!(typeof(data[i].value)=='undefined')){
                    ...
                    if (animationDecimal >= 1) {
                        ...
                        jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["ARC", midPieX, midPieY, cutoutRadius, doughnutRadius, cumulativeAngle - segmentAngle, cumulativeAngle, data[i].answers, 1*data[i].value, cumvalue, totvalue, segmentAngle, i];
                        if (config.inGraphDataShow) {
                            if(config.inGraphDataAnglePosition==1)posAngle=realCumulativeAngle+config.inGraphDataPaddingAngle*(Math.PI/180);
                            else if(config.inGraphDataAnglePosition==2)posAngle=realCumulativeAngle-segmentAngle/2+config.inGraphDataPaddingAngle*(Math.PI/180);
                            else if(config.inGraphDataAnglePosition==3)posAngle=realCumulativeAngle-segmentAngle+config.inGraphDataPaddingAngle*(Math.PI/180);
...
//X Labels
            if(config.xAxisTop || config.xAxisBottom) {
                ...
                if(config.xAxisBottom){
                    for (var i = 0; i < data.labels.length; i++) {
                        ctx.fillStyle = data.labelsfillColor[i];
                        ctx.save();
                ...
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    ...
                    if (!(typeof(data.datasets[i].data[j])=='undefined')) {
                        ...
                        if (animPc >= 1) {
                           ...
                           // console.log(totvalue[j]);
                            //console.log(cumvalue[j]);

                            jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", barOffset, t1, barOffset + barWidth, t2, data.datasets[i].answers[j],
                                1*data.datasets[i].data[j],'('+ lgtxt2 +')',cumvalue[j], totvalue[j], i, j];
                        }
                    }
                }
            }
                ...
                for (var j = 0; j < data.datasets[i].data.length; j++) {
                    ...
                    if (!(typeof(data.datasets[i].data[j])=='undefined')) {
                        ...
                        if (animPc >= 1) {
                            ...
                            if (t1 > t2) { t3 = t1; t1 = t2; t2 = t3 }
                            jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["RECT", t1, barOffset + barWidth, t2, barOffset, data.datasets[i].answers[j] ,  1*data.datasets[i].data[j], '('+ lgtxt2 + ')', cumvalue[j], totvalue[j], i, j];
                        }
                    }
                }
            }
...
// next not missing value
                        ...
                        if (animPc >= 1) {
                            ...
                            jsGraphAnnotate[ctx.ChartNewId][jsGraphAnnotate[ctx.ChartNewId].length] = ["POINT", xPos(i,j,data), yPos(i, j), data.datasets[i].answers[j],  1*data.datasets[i].data[j],'('+lgtxt2 +')', divprev, divnext, maxvalue[j], totvalue[j], i, j];
                            if (config.inGraphDataShow) {
                                ctx.save();
                                ctx.textAlign = config.inGraphDataAlign;
                                ctx.textBaseline = config.inGraphDataVAlign;
